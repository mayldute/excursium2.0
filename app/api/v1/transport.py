...
@router.get(
    "/search",
    response_model=List[TransportFilterResponse],
...
@router.get(
    "/search/{transport_id}",
    response_model=TransportFilterResponse,
    summary="Search transports by carrier ID",
    status_code=200
)
...
async def create_trancport(
    ...
    return await create_transport_service(
        payload, context.current_user, context.db
    )
...
@router.get(
    ...
    summary="Get transport by carrier ID",
    status_code=200
)
async def get_transport_by_carrier_id(
    ...
    return await get_transports_by_carrier_id_service(
...
async def update_transport(
    ...
    return await update_transport_service(
        transport_id, payload, context.current_user, context.db
    )
...
async def delete_transport(
    ...
    return await delete_transport_service(
        transport_id, context.current_user, context.db
    )


@router.patch(
    "/photo/{transport_id}",
    summary="Update transport photo",
    status_code=200
)
async def update_transport_photo(
    ...
    return await upload_transport_photo_service(
        transport_id, file, context.current_user, context.db
    )


@router.post(
    "/add_route/{transport_id}",
    summary="Add route to transport",
    status_code=201
)
async def add_route_to_transport(
    ...
    return await add_route_to_transport_service(
        transport_id,
        id_from,
        id_to,
        min_price,
...
@router.get(
    "/routes/{transport_id}",
    summary="Get routes by transport ID",
    status_code=200
)
async def get_routes_by_transport_id(
    ...
    return await get_transport_routes_service(
        transport_id, context.current_user, context.db
    )


@router.delete(
    "/route/{transport_id}/{route_id}",
    summary="Delete route from transport",
    status_code=204
)
async def delete_route_from_transport(
    ...
    return await delete_transport_route_service(
        transport_id, route_id, context.current_user, context.db
    )
...
async def add_schedule_to_transport(
    ...
    return await add_transport_schedule_service(
        payload, context.current_user, context.db
    )
...
@router.get(
    ...
    summary="Get schedules by transport ID",
    status_code=200
)
async def get_schedules_by_transport_id(
    ...
    return await get_transport_schedules_service(
        transport_id, context.current_user, context.db
    )
...
@router.delete(
    ...
    summary="Delete schedule from transport",
    status_code=204
)
async def delete_schedule_from_transport(
    ...
    return await delete_transport_schedules_service(
        schedule_id, context.current_user, context.db
    )
