...
@router.post(
    "/client",
    summary="Client registration (individual/legal entity)",
    status_code=201
)
...
async def update_client(
    ...
    return await update_client_service(
        client_id, payload, context.current_user, context.db
    )
...
async def delete_client(
    ...
    return await delete_client_service(
        client_id, context.current_user, context.db
    )


@router.get(
    "/login/{provider}",
    summary="Initiate OAuth login with provider",
    status_code=200
)
...
@router.patch(
    ...
    summary="Complete client registration after social login",
    status_code=200
)
async def complete_registration(
    ...
    return await complete_social_registration(
