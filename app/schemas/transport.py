...
class TransportFilter(BaseModel):
    ...
    start_time: datetime = Field(..., description="Start time", example="2025-06-05T08:00:00Z")
    end_time: datetime = Field(..., description="End time", example="2025-06-05T12:00:00Z")
    n_seat: int = Field(..., gt=0, description="Number of seats", example=10)
    min_price: float = Field(..., ge=0, description="Minimum price", example=100.0)
    max_price: float = Field(..., ge=0, description="Maximum price", example=500.0)
    ...
    def validate_times_and_prices(self) -> "TransportFilter":
        ...
        if errors:
            raise ValidationError.from_exception_data(self.__class__.__name__, [ (k, v) for k, v in errors.items() ])

        return self

class TransportFilterResponse(BaseModel):
    id: int = Field(..., description="Transport ID", example=1)
    carrier_id: int = Field(..., description="ID of the carrier providing the transport", example=1)
    ...
    min_price: float = Field(..., description="Minimum price of the transport", example=100.0)
    max_price: float = Field(..., description="Maximum price of the transport", example=500.0)
...
class TransportUpdate(BaseModel):
    name: str | None = Field(None, description="Updated name of the transport", example="Bus-456")
    brand: str | None = Field(None, description="Updated brand", example="Ford")
    model: str | None = Field(None, description="Updated model", example="Transit Custom")
    year: int | None = Field(None, description="Updated year of manufacture", example=2022)
    n_seat: int | None = Field(None, description="Updated number of total seats", example=20)
    luggage: bool | None = Field(None, description="Update luggage availability")
    ...
    air_conditioning: bool | None = Field(None, description="Update air conditioning availability")
