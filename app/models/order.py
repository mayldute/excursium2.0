...
class Order(Base):
    ...
    passenger_type: Mapped[PassengerTypeEnum] = mapped_column(
        Enum(PassengerTypeEnum), nullable=False
    )
    ...
    comment: Mapped["Comment"] = relationship(
        "Comment", back_populates="order", uselist=False
    )
    extra_services: Mapped[list["ExtraService"]] = relationship(
        "ExtraService", back_populates="order", lazy='dynamic', uselist=True
    )
    ...
    transport: Mapped["Transport"] = relationship(
        "Transport", back_populates="orders"
    )
    id_route: Mapped[int] = mapped_column(
        Integer, ForeignKey("routes.id", ondelete="CASCADE")
    )
...
class OrderHistory(Base):
    ...
    old_status: Mapped[OrderStatusEnum | None] = mapped_column(
        Enum(OrderStatusEnum), nullable=True
    )
    new_status: Mapped[OrderStatusEnum] = mapped_column(
...
class Comment(Base):
    ...
    order: Mapped["Order"] = relationship(
        "Order", back_populates="comment", uselist=False
    )
    carrier_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("carriers.id", ondelete="CASCADE")
    )
    carrier: Mapped["Carrier"] = relationship(
        "Carrier", back_populates="comments"
    )
    transport_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("transports.id", ondelete="CASCADE")
    )
    transport: Mapped["Transport"] = relationship(
        "Transport", back_populates="comments"
    )
